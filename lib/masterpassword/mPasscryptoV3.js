...
export const generateAuthData = (masterPass, existingSalt = null) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onerror = () => {
      worker.terminate();
      if (!navigator.onLine) {
        reject(new Error("You are offline. Unable to load the encryption module."));
      } else {
        reject(new Error("Worker thread failed to execute: " + (errorEvent.message || "Unknown error")));
      }
    };
  });
};