...
import { handleResetVaultEmailSend } from "@/lib/user/resetvault/resetVaultEmailSend";

export default function ResetVaultModal({
  isOpen,
  onClose,
  showPasswordInput,
}) {
  ...
  const handleReset = async () => {
    setIsResetting(true);
    await toast.promise(handleResetVaultEmailSend(confirmationText), {
      loading: "Sending...",
      success: (data) => {
  ...
  return (
    <div className="fixed min-h-screen inset-0 z-50 flex justify-center bg-gray-950/70 dark:bg-black/80 backdrop-blur-md overflow-auto scroll-bar-hide ">
      <div className="relative bg-white dark:bg-gray-800 p-4 md:p-8 rounded-2xl shadow-2xl w-[95%] max-w-md max-h-[80vh] animate-scale-in border-3 border-gray-200 dark:border-gray-700 overflow-auto scroll-bar-hide my-auto">
        {/* Close Button */}
        <X
          onClick={onClose}
          className="absolute top-4 right-4 w-6 h-6 text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors duration-300 cursor-pointer"
          role="button"
        ...
        <p className="text-base md:text-lg text-gray-700 dark:text-gray-300 mb-6 leading-relaxed">
          ...
          All your saved entries{" "}
          <span className="font-bold text-red-600 dark:text-red-400">
            including your master password
          </span>{" "}
          will be{" "}
          <span className="font-bold text-red-600 dark:text-red-400">
            permanently deleted
        ...
        {showPasswordInput && (
          <div className="relative group mb-6 text-left">
            ...
            <input
              ...
              placeholder=" "
              className="peer w-full px-4 py-2.5 border-2 rounded-lg bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-100 placeholder-transparent outline-none focus:border-red-500 dark:focus:border-red-400 transition duration-300 text-lg"
            />
          </div>
        )}
        ...
        <div className="flex gap-4 md:mt-8 flex-col-reverse md:flex-row">
          ...
          <button
            onClick={handleReset}
            disabled={
              isResetting || (showPasswordInput && !confirmationText.trim())
            }
